...
#### elastic-net
elastic.PSM.hcq <- lapply(1:Imputations, function(im1) {
  #im1 <- 1
...
V1 <- table(unlist(elastic.PSM.hcq))
names(V1[V1>=9])[-1] # ignore intercept

#[1] "age_cph"                       "ecogcat22+"                    "severity_of_covid_19_v2Severe" "treatment_hcqHCQ+anything"           
...
elastic.IPTW.hcq <- lapply(1:Imputations, function(im1) {
  #im1 <- 1
  full_dat1 <- fill_data(im = im1) # complete data
...
V1 <- table(unlist(elastic.IPTW.hcq))
names(V1[V1>=9])[-1] # ignore intercept
